...
-- 연습문제 
-- 1. DISTINCT, ORDER BY, ASC, DESC
-- 2. "%45"
-- 3. "__34%"
-- 4. "%34%"
-- ‘서울’에 사는 고객 중에 마일리지가 15,000점 이상 20,000점 이하인 고객의 모든 컬럼 정보를 보이시오.
SELECT * 
FROM 고객
WHERE 도시 LIKE "%서울%" AND 15000<= 마일리지 < 20000;

-- 세계무역의 고객들은 어느 지역, 어느 도시에 사는지 지역과 도시를 한 번씩만 보이시오. 이때 결과를 지역 순으로 나타내고, 동일 지역에 대해서는 도시 순으로 나태내시오.
...
-- ‘춘천시’나 ‘과천시’ 또는 ‘광명시’에 사는 고객 중에서 담당자직위에 ‘이상’ 또는 ‘사원’이 들어가는 고객의 모든 정보를 보이시오.
SELECT *
FROM 고객
WHERE 도시 IN ("춘천시","과천시","광명시") AND (담당자직위 LIKE "%이사" OR 담당자직위 LIKE "%사원");

-- ‘광역시’나 ‘특별시’에 살지 않는 고객들 중에서 마일리지가 많은 상위 고객 3명의 모든 정보를 출력하시오.
SELECT *
FROM 고객
WHERE 도시 NOT LIKE "%광역시" AND 도시 NOT LIKE "%특별시" -- 둘다 아니여야 되니까 AND
ORDER BY 마일리지 DESC
LIMIT 3;
...
SELECT  *
FROM 고객
WHERE 지역 IS NOT NULL AND 담당자직위 != "대표읻사"; -- <>도 가능
