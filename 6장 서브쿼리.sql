...
-- 1. ‘배재용’ 사원의 부서명을 보이시오.
SELECT * FROM 사원;
SELECT 이름, (SELECT 부서명 FROM 부서 WHERE 사원.부서번호 = 부서.부서번호) AS 부서명
FROM 사원
WHERE 이름 = '배재용';
...
SELECT * FROM 고객;
SELECT * FROM 주문;
SELECT 담당자명, 고객회사명,COUNT(주문번호) AS 주문건수, MIN(주문일) AS 최초주문일, MAX(주문일) AS 최종주문일
FROM 고객 
JOIN 주문
ON 고객.고객번호 = 주문.고객번호
GROUP BY 담당자명, 고객회사명;
...
SELECT * FROM 고객;
SELECT * FROM 주문;
SELECT 도시,YEAR(주문일) AS 주문년도, COUNT(주문.주문일) AS 주문건수
